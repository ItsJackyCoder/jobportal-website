...
import com.jackycoder.jobportal.entity.IRecruiterJobs;
...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    //add an additional supporting method
    @Query(value = "SELECT " +
            ...
            "ORDER BY j.posted_date DESC", nativeQuery = true)
    List<IRecruiterJobs> getRecruiterJobs(@Param("recruiter") int recruiter);

    @Query(value = "SELECT * " +
            "FROM job_post_activity j " +
            "INNER JOIN job_location l on j.job_location_id=l.id  WHERE j.job_title " +
            "LIKE %:job% " +
